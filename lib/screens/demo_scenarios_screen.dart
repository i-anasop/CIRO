...
class _DemoScenariosScreenState extends State<DemoScenariosScreen> {
  ...
  Future<void> _activate(BuildContext ctx, DemoScenario s) async {
    setState(() { _loading = true; _activatingId = s.id; });

    await ScenarioEngine.instance.selectScenario(s.id);

    if (!ctx.mounted) return;
    setState(() { _loading = false; _activatingId = null; });

    ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
      content: Row(children: [
        const Icon(Icons.check_circle_rounded, color: Colors.white, size: 16),
        const SizedBox(width: 8),
        Expanded(child: Text('Scenario activated: ${s.title}',
            style: CiroTypography.bodySmall.copyWith(color: Colors.white))),
      ]),
      backgroundColor: CiroColors.success,
      ...
      builder: (context, _) {
        ...
        return Scaffold(
          ...
          body: ListView(
            ...
            children: [
              // ── Header ────────────────────────────────────────────────────
              Text('Preloaded Scenarios', style: CiroTypography.headingMedium),
              const SizedBox(height: 4),
              Text(
                'Tap "Launch" to activate a scenario. '
              ...
              // ── Scenario cards ────────────────────────────────────────────
              ...scenarios.map((s) {
                final sColor   = CiroColors.bySeverity(_severityLabel(s.severity));
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isActive ? CiroColors.brandAccent : CiroColors.border,
                      width: isActive ? 1.5 : 1,
                    ),
                    boxShadow: isActive ? CiroColors.glowCyan : CiroColors.cardShadow,
                  ),
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            if (isActive)
                              Container(
                                padding: const EdgeInsets.symmetric(
                                    horizontal: 8, vertical: 3),
                                decoration: BoxDecoration(
                                  color: CiroColors.success.withValues(alpha: 0.12),
                                  borderRadius: BorderRadius.circular(CiroSpacing.radiusXs),
                                ),
                                child: Text('ACTIVE',
                                    style: CiroTypography.overline.copyWith(
                          ...
                          Row(children: [
                            ...
                            Text(s.location, style: CiroTypography.bodySmall),
                          ]),
                          const SizedBox(height: CiroSpacing.md),

                          // Severity + confidence + verification
                          Wrap(spacing: CiroSpacing.sm, runSpacing: CiroSpacing.sm, children: [
                            SeverityBadge(label: _severityLabel(s.severity)),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                              decoration: BoxDecoration(
                                color: CiroColors.brandAccent.withValues(alpha: 0.08),
                                borderRadius: BorderRadius.circular(CiroSpacing.radiusXs),
                              ),
                              child: Text('${s.confidence.toInt()}% conf.',
                                  style: CiroTypography.overline.copyWith(
                                      color: CiroColors.brandAccent)),
                            ),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                              decoration: BoxDecoration(
                                color: _verificationColor(s.verificationType).withValues(alpha: 0.10),
                                borderRadius: BorderRadius.circular(CiroSpacing.radiusXs),
                              ),
                              child: Text(s.verificationLabel,
                                  style: CiroTypography.overline.copyWith(
                                      color: _verificationColor(s.verificationType),
                                      fontSize: 9)),
                            ),
                          ]),
                          ...
                          Row(children: [
                            ...
                            const Icon(Icons.people_outline,
                                size: 12, color: CiroColors.textMuted),
                            const SizedBox(width: 3),
                            Text('${_fmt(s.affectedPopulation)} affected',
                                style: CiroTypography.bodySmall),
                          ]),
                          const SizedBox(height: CiroSpacing.md),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Row(children: [
                      const Icon(Icons.info_outline, color: CiroColors.warning, size: 16),
                      const SizedBox(width: CiroSpacing.sm),
                      Text('False Signal Handling',
  ...
  String _srcShort(SignalSource s) {
    switch (s) {
      case SignalSource.socialPost:    return 'Social';
      case SignalSource.weatherAlert:  return 'Weather';
      case SignalSource.trafficData:   return 'Traffic';
      case SignalSource.mockSensor:    return 'Sensor';
