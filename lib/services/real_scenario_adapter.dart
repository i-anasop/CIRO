...
class RealScenarioAdapter {
  ...
  static DemoScenario fromBundle(RealSignalBundle bundle) {
    ...
    final publicSignalCount =
        bundle.newsSignals.length + bundle.socialPosts.length;
    final socialSignal = SignalInput(
      source: SignalSource.socialPost,
      content: publicSignalCount > 0
          ? 'Live public feed: $publicSignalCount relevant signal(s). Latest: "${bundle.socialPosts.isNotEmpty ? bundle.socialPosts.first.text : bundle.newsSignals.first.title}" (${bundle.socialPosts.isNotEmpty ? bundle.socialPosts.first.handle : bundle.newsSignals.first.source}).'
          : 'Live public/news feed: no crisis keywords found near $location.',
      confidence: publicSignalCount > 0 ? 0.80 : 0.55,
      isActive: publicSignalCount > 0 || active,
    );
    ...
    final warnings = [
      ...
      ...bundle.warnings,
      if (bundle.newsSignals.isEmpty)
        'News/public feed returned no relevant local crisis articles',
    ];
    ...
    return DemoScenario(
      ...
      resourceSummary: active
          ? _resourceSummary(type)
          : 'All units standby; live monitoring active',
      ...
      orchestration: ScenarioOrchestrationHints(
        ...
        affectedRadius: active
            ? _radius(type, severity)
            : '0.5 km monitoring radius',
  ...
  static _Classification _classify(RealSignalBundle bundle) {
    ...
    final isInjected = bundle.warnings.any(
      (w) => w.contains('Simulated threat'),
    );
    ...
    final hasActiveFloodWeather =
        weather?.alertLevel == WeatherRisk.floodRisk || rainfall > 12.0;
    final hasLocalFloodNews =
        (text.contains('flood') || text.contains('waterlog')) &&
        isLocallySpecific(text);
    ...
    final hasActiveHeatWeather =
        weather?.alertLevel == WeatherRisk.heatwave ||
        temp >= 39.0 ||
        feels >= 42.0;
    final hasLocalHeatNews =
        text.contains('heatwave') && isLocallySpecific(text);
    ...
    final outage =
        (text.contains('power outage') ||
            text.contains('blackout') ||
            text.contains('load shedding')) &&
        isLocallySpecific(text);
    ...
    final hasLocalBlockNews =
        text.contains('road blocked') && isLocallySpecific(text);
    final blocked = hasSevereTraffic || hasLocalBlockNews;

    if (flood) {
      return _Classification(
        CrisisType.urbanFlooding,
        rainfall > 20 || weather?.alertLevel == WeatherRisk.floodRisk
  ...
  static double _confidence(
    ...
    if (bundle.weather?.isSuccess == true) {
      score += weatherBoost == 0 ? 6 : weatherBoost;
    }
    if (bundle.traffic?.isSuccess == true) {
      score += trafficBoost == 0 ? 5 : trafficBoost;
    }
    ...
    if (!active) {
      signals.add(
        SignalInput(
          ...
          content:
              'CIRO-derived local sensor baseline for $area: weather and route readings are inside normal limits.',
          confidence: 0.70,
          isActive: true,
        ),
      );
      ...
      case CrisisType.urbanFlooding:
        signals.add(
          SignalInput(
            source: SignalSource.mockSensor,
            content:
                'CIRO-derived flood sensor estimate for $area: rainfall/runoff risk elevated from live weather and route delay.',
            confidence: 0.82,
          ),
        );
        signals.add(
          SignalInput(
            ...
            content:
                'CIRO-derived call-frequency estimate: flood/stranding call volume expected above baseline in $area.',
            confidence: 0.74,
          ),
        );
        break;
      case CrisisType.heatwave:
        signals.add(
          SignalInput(
            source: SignalSource.mockSensor,
            content:
                'CIRO-derived heat sensor estimate for $area: live temperature/feels-like crosses health-risk threshold.',
            confidence: 0.86,
          ),
        );
        signals.add(
          SignalInput(
            ...
            content:
                'CIRO-derived field advisory: hydration/cooling outreach recommended near dense outdoor activity zones.',
            confidence: 0.72,
          ),
        );
        break;
      case CrisisType.accident:
        signals.add(
          SignalInput(
            source: SignalSource.emergencyCall,
            content:
                'CIRO-derived emergency-call proxy: accident keywords plus route delay suggest dispatch verification.',
            confidence: 0.75,
          ),
        );
        break;
      case CrisisType.powerOutage:
        signals.add(
          SignalInput(
            source: SignalSource.mockSensor,
            content:
                'CIRO-derived infrastructure sensor proxy: outage keywords require utility confirmation for $area.',
            confidence: 0.70,
          ),
        );
        break;
      case CrisisType.roadBlockage:
        signals.add(
          SignalInput(
            source: SignalSource.fieldReport,
            content:
                'CIRO-derived field task: verify blockage cause and confirm whether towing or traffic control is needed.',
            confidence: 0.73,
          ),
        );
        break;
    ...
    if (severity == SeverityLevel.critical) {
      signals.add(
        SignalInput(
          source: SignalSource.fieldReport,
          content:
              'CIRO-derived critical escalation: field commander review required within 10 minutes.',
          confidence: 0.78,
        ),
      );
    }
    ...
    switch (type) {
      case CrisisType.urbanFlooding:
        return [
          PlanAction(
            step: 1,
            title: 'Reroute Traffic Around $location',
            description:
                'Use live route delays to divert vehicles away from low-lying corridors.',
            department: 'Traffic Authority',
            priority: 'P1',
            eta: '3 min',
            status: 'In Progress',
            resultSummary:
                'Traffic diversion plan generated from live route signal.',
          ),
          PlanAction(
            step: 2,
            title: 'Dispatch Drainage and Rescue Units',
            description:
                'Send pump/rescue crew to highest-risk catchment near the user location.',
            department: 'Rescue / Municipal Drainage',
            priority: 'P1',
            eta: '10 min',
            status: 'Pending',
          ),
          PlanAction(
            step: 3,
            title: 'Send Localized Flood Alert',
            description:
                'Warn approximately $affected nearby residents and commuters.',
            department: 'Emergency Broadcast',
            priority: 'P2',
            eta: '5 min',
            status: 'Completed',
          ),
      ...
      case CrisisType.heatwave:
        return [
          PlanAction(
            step: 1,
            title: 'Open Cooling and Hydration Points',
            description:
                'Deploy water/cooling support around dense public zones near $location.',
            department: 'Health / Municipal Services',
            priority: 'P1',
            eta: '15 min',
            status: 'In Progress',
          ),
          PlanAction(
            step: 2,
            title: 'Notify Nearby Hospitals',
            description:
                'Prepare heatstroke triage for at-risk population estimate of $affected.',
          ...
          PlanAction(
            step: 3,
            title: 'Broadcast Heat Advisory',
            description:
                'Send public health guidance based on live temperature thresholds.',
            department: 'Emergency Broadcast',
            priority: 'P2',
            eta: '3 min',
            status: 'Completed',
      ...
      case CrisisType.accident:
        return [
          PlanAction(
            step: 1,
            title: 'Dispatch Ambulance and Traffic Police',
            description:
                'Use live congestion and accident keywords to verify and secure scene near $location.',
            department: 'Rescue / Traffic Police',
            priority: 'P1',
            eta: '6 min',
            status: 'In Progress',
          ),
          PlanAction(
            ...
            description:
                'Reduce secondary collision risk by diverting approach traffic.',
            department: 'Traffic Authority',
            priority: 'P1',
            eta: '4 min',
            status: 'Completed',
      ...
      case CrisisType.powerOutage:
        return [
          PlanAction(
            step: 1,
            title: 'Escalate Utility Ticket',
            description:
                'Send outage evidence and location to the utility response desk.',
            department: 'Utility Provider',
            priority: 'P1',
            eta: '8 min',
            status: 'In Progress',
          ),
          PlanAction(
            step: 2,
            title: 'Check Critical Backup Power',
            description:
                'Prioritize clinics, signals, and vulnerable facilities near $location.',
            department: 'Emergency Utilities',
            priority: 'P2',
            eta: '15 min',
            status: 'Pending',
          ),
      ...
      case CrisisType.roadBlockage:
        return [
          PlanAction(
            step: 1,
            title: 'Deploy Traffic Control to $location',
            description:
                'Confirm blockage and open a managed lane if possible.',
          ...
          PlanAction(
            step: 2,
            title: 'Publish Alternate Route Advisory',
            description: 'Use live route delay to redirect nearby commuters.',
  ...
  static List<PlanAction> _monitoringActions(String location) => [
    PlanAction(
      step: 1,
      title: 'Continue Live Monitoring at $location',
      description:
          'Poll location, weather, public/news, and route signals for threshold changes.',
      department: 'CIRO Live Monitor',
      priority: 'P3',
      eta: 'Ongoing',
      status: 'Completed',
    ),
    const PlanAction(
      step: 2,
      title: 'Keep Fallback Demo Pipeline Ready',
      description:
          'Use mock-first scenario if APIs become unavailable during judging.',
      department: 'CIRO Runtime',
      priority: 'P3',
      eta: 'Ongoing',
      status: 'Completed',
    ),
  ...
  static List<MetricPair> _metrics(
    CrisisType type,
    SeverityLevel severity,
    int affected,
    RouteResult? traffic,
  ) {
    final congestionBefore =
        traffic?.congestionPercent ??
        (type == CrisisType.roadBlockage ? 75 : 45);
    ...
    return [
      MetricPair(
        label: 'Congestion %',
        before: '$congestionBefore%',
        after: '$congestionAfter%',
        delta: 'down ${congestionBefore - congestionAfter}%',
        isImprovement: true,
      ),
      MetricPair(
        label: 'Response Time',
        before: '$responseBefore min',
        after: '$responseAfter min',
        delta: 'down ${responseBefore - responseAfter} min',
        isImprovement: true,
      ),
      MetricPair(
        label: 'Risk Level',
        before: _severityLabel(severity),
        after: severity == SeverityLevel.critical ? 'High' : 'Moderate',
        delta: 'down',
        isImprovement: true,
      ),
      MetricPair(
        ...
        before: '$affected',
        after: '${(affected * 0.34).round()}',
        delta: 'down ${(affected * 0.66).round()}',
        isImprovement: true,
      ),
    ];
  }

  static int _affectedPopulation(
    CrisisType type,
    SeverityLevel severity,
    RouteResult? traffic,
  ) {
    ...
    final sev = severity == SeverityLevel.critical
        ? 1.45
        : severity == SeverityLevel.high
        ? 1.15
        : 0.70;
    final trafficBoost = (traffic?.congestionLevel == CongestionLevel.high)
        ? 1.25
        : 1.0;
    return (base * sev * trafficBoost).round();
  }

  static VerificationType _verification(
    RealSignalBundle bundle,
    bool active,
    double confidence,
  ) {
    if (!active) return VerificationType.needsVerification;
    if (confidence >= 78 && bundle.signalCount >= 2) {
      return VerificationType.confirmed;
    }
    if (bundle.warnings.isNotEmpty) return VerificationType.needsVerification;
    return VerificationType.conflictingSignals;
  }

  static String _verificationNote(
    RealSignalBundle bundle,
    bool active,
    double confidence,
  ) {
    ...
    final warnings = bundle.warnings.isEmpty
        ? 'No service warnings.'
        : 'Service warnings: ${bundle.warnings.join("; ")}.';
  ...
  static List<String> _resources(CrisisType type) => switch (type) {
    CrisisType.urbanFlooding => [
      'Drainage Pump DP-REAL',
      'Rescue Unit RU-REAL',
      'Traffic Unit TU-REAL',
    ],
    CrisisType.heatwave => [
      'Medical Team MT-REAL',
      'Water Tanker WT-REAL',
      'Hospital Desk HD-REAL',
    ],
    CrisisType.accident => [
      'Ambulance AMB-REAL',
      'Traffic Patrol TP-REAL',
      'Tow Truck TT-REAL',
    ],
    CrisisType.powerOutage => [
      'Utility Crew UC-REAL',
      'Generator GEN-REAL',
      'Field Team FT-REAL',
    ],
  ...
  static String _resourceSummary(CrisisType type) => switch (type) {
    CrisisType.urbanFlooding =>
      'Drainage pump, rescue unit, and traffic diversion team',
    CrisisType.heatwave =>
      'Medical outreach team, water support, and hospital preparedness',
    CrisisType.accident => 'Ambulance, traffic patrol, and towing support',
    CrisisType.powerOutage =>
      'Utility repair crew, generator support, and field verification',
    CrisisType.roadBlockage => 'Traffic patrol and towing support',
  };

  static Map<String, String> _stakeholders(
    CrisisType type,
    String location,
    bool active,
    ...
    if (!active) {
      return {
        'Public':
            'No active crisis confirmed near $location. Continue normal movement and monitor CIRO updates.',
        'Command Center':
            'Real Mode monitoring active. ${warnings.isEmpty ? "All available sources normal." : warnings.join("; ")}',
      };
    }
    return {
      'Public':
          '${_typeLabel(type)} risk near $location. Follow localized instructions and avoid affected corridors.',
      'Emergency Services':
          'Dispatch and verify ${_typeLabel(type)} response near $location using live location evidence.',
      'Hospitals':
          'Prepare capacity proportional to CIRO affected-population estimate near $location.',
      'Utilities':
          'Review supporting infrastructure risks for ${_typeLabel(type)} near $location.',
      'Transport Authority':
          'Use live route impact to stage rerouting and prevent congestion spillover.',
      'Command Center':
          'Real Mode trace available. ${warnings.isEmpty ? "No degraded sources." : warnings.join("; ")}',
    };
  }
  ...
  static List<String> _tradeOffs(CrisisType type, List<String> warnings) => [
    ...
    if (warnings.isNotEmpty)
      'Confidence widened because ${warnings.length} live source warning(s) occurred.',
  ...
  static String _radius(CrisisType type, SeverityLevel severity) {
    ...
    return switch (type) {
      CrisisType.urbanFlooding =>
        critical ? '2.5 km drainage basin' : '1.5 km local basin',
      CrisisType.heatwave =>
        critical ? '5 km heat exposure zone' : '3 km heat exposure zone',
  ...
  static String _spreadRisk(CrisisType type) => switch (type) {
    CrisisType.urbanFlooding =>
      'High if rainfall continues or drains are blocked.',
    CrisisType.heatwave =>
      'High for outdoor workers, elderly people, and dense neighborhoods.',
    CrisisType.accident => 'Moderate secondary collision and congestion risk.',
    CrisisType.powerOutage =>
      'Moderate risk to clinics, signals, water pumps, and security.',
    CrisisType.roadBlockage => 'Moderate-to-high congestion spillover risk.',
  };

  static String _evolution(
    CrisisType type,
    SeverityLevel severity,
    String location,
  ) {
